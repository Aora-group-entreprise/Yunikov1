...
function signAccessToken(userId: string, sessionId: string): string {
  ...
  const payload = Buffer.from(JSON.stringify({ sub: userId, sid: sessionId, iat: Math.floor(Date.now() / 1000), exp: Math.floor(Date.now() / 1000) + ACCESS_TTL_SECONDS })).toString("base64url");
...
async function hashPassword(password: string): Promise<string> {
  ...
  const derived = await new Promise<Buffer>((resolve, reject) => {
    crypto.scrypt(password, salt, 64, { N: 16384, r: 8, p: 2 }, (error, key) => error ? reject(error) : resolve(key));
  });
...
async function verifyPassword(password: string, encoded: string): Promise<boolean> {
  ...
  const actual = await new Promise<Buffer>((resolve, reject) => {
    crypto.scrypt(password, salt, expected.length, { N: 16384, r: 8, p: 2 }, (error, key) => error ? reject(error) : resolve(key));
  });
...
function validateUsername(username: string): void {
  if (!/^[A-Za-z0-9_]{3,32}$/.test(username)) throw new Error("invalid_username");
}
...
async function createSession(userId: string, ip?: string, userAgent?: string): Promise<AuthSession> {
  ...
  await pool.query(
    `insert into yunikov_v1.auth_sessions (id,user_id,refresh_token_hash,ip,user_agent,expires_at) values ($1,$2,$3,$4,$5,$6)`,
    [sessionId, userId, hashToken(refreshToken), ip ?? null, userAgent ?? null, new Date(Date.now() + REFRESH_TTL_MS)],
  );
...
export async function registerUser(input: RegisterInput, meta: { ip?: string; userAgent?: string } = {}): Promise<AuthSession> {
  ...
  try {
    ...
    const userResult = await client.query<{ id: string }>(
      `insert into yunikov_v1.users (email) values (lower($1)) returning id`,
      [input.email.trim()],
    );
    ...
    await client.query(`alter table yunikov_v1.users add column if not exists password_hash text`);
    await client.query(`update yunikov_v1.users set password_hash=$2 where id=$1`, [userId, passwordHash]);
...
export async function signIn(identifier: string, password: string, meta: { ip?: string; userAgent?: string } = {}): Promise<AuthSession> {
  const result = await pool.query<{ id: string; password_hash: string | null; status: string }>(
    `select id,password_hash,status from yunikov_v1.users u left join yunikov_v1.profiles p on p.id=u.id where lower(u.email)=lower($1) or lower(p.username)=lower($1) limit 1`,
    [identifier.trim()],
  );
  const row = result.rows[0];
  const valid = !!row?.password_hash && row.status === "active" && await verifyPassword(password, row.password_hash);
  if (!valid) {
    await pool.query(`insert into yunikov_v1.login_events (user_id,ip,user_agent,is_new_device) values ($1,$2,$3,false)`, [row?.id ?? null, meta.ip ?? null, meta.userAgent ?? null]);
    throw new Error("invalid_credentials");
  }
  await pool.query(`insert into yunikov_v1.login_events (user_id,ip,user_agent,is_new_device) values ($1,$2,$3,false)`, [row.id, meta.ip ?? null, meta.userAgent ?? null]);
...
export async function signOut(sessionId: string, scope: "current" | "all"): Promise<void> {
  if (scope === "all") {
    await pool.query(`update yunikov_v1.auth_sessions set revoked_at=now() where user_id=(select user_id from yunikov_v1.auth_sessions where id=$1) and revoked_at is null`, [sessionId]);
  } else {
    await pool.query(`update yunikov_v1.auth_sessions set revoked_at=now() where id=$1`, [sessionId]);
  }
}
...
export async function requestPasswordReset(identifier: string): Promise<void> {
  // The response is deliberately neutral. A production mail provider can consume this event later.
