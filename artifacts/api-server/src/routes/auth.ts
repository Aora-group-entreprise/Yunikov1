...
function issue(res:Response,session:{refreshToken:string;accessToken:string;userId:string;sessionId:string}){res.cookie(REFRESH_COOKIE,session.refreshToken,{...cookieOptions,maxAge:30*24*60*60*1000});return{userId:session.userId,sessionId:session.sessionId,accessToken:session.accessToken};}
function authError(error:unknown):{status:number;body:{error:string}}{const code=error instanceof Error?error.message:"auth_error";if(["password_too_short","password_too_weak","invalid_username"].includes(code))return{status:400,body:{error:code}};if(code==="account_already_exists")return{status:409,body:{error:code}};if(["invalid_credentials","invalid_refresh_token"].includes(code))return{status:401,body:{error:code}};if(code==="too_many_attempts")return{status:429,body:{error:code}};if(code==="auth_session_secret_not_configured")return{status:503,body:{error:"auth_not_configured"}};return{status:500,body:{error:"auth_error"}};}
